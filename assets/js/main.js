...
//     event.preventDefault();
//     if(event.type === 'click' || (event.type === 'keydown') && (event.keyCode === 13 || event.keyCode === 32)){
//         /*Need to implement in feature*/
//         var scrollElement = $(this).attr('data-top');
//         if(!$(this).hasClass('activeblack')){
//             var position = $('#'+$(this).attr('data-top')).position().top ;
//             if(position === NaN || position < 0){
//                 position = $('#'+$(this).attr('data-top')).offset().top;
//                 if(position<-100){
//                     position = position * (-1);
//                 }
//             }
...
$(document).on('click keydown', '.colorBlue', function (event) {
    if(event.type === 'click' || (event.type === 'keydown') && (event.keyCode === 13 || event.keyCode === 32)){
        event.preventDefault();
        $('.slide1Disclaimer').css("display", "block");
        removeTabIndex();
...
$(document).on('click keydown', '#closeIcon', function (event) {
    if(event.type === 'click' || (event.type === 'keydown') && (event.keyCode === 13 || event.keyCode === 32)){
        event.preventDefault();
        $('.slide1Disclaimer').css("display", "none");
        setTabIndex();
        lastFocusedElement.focus();
    }
});
...
function addAccessibilityRolesAndTab(){
    ...
    // $('.slides div[class *= "link"').addClass('tabindex').attr('role','menuitem');
